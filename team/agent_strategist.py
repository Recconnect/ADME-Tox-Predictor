"""
Strategist Agent — синтезирует 6 аудитов команды в единый GTM-план для РФ.

Запуск:
    python -m team.agent_strategist
"""
...
from team.config import (
    AUDIT_DIR, STRATEGY_DIR,
    RUSSIAN_PHARMA_COMPANIES, RUSSIAN_CRO_COMPANIES,
    RUSSIAN_GRANTS, VCS_RUSSIAN, AI_RUSSIAN_COMPETITORS,
)

START_DATE = datetime(2026, 7, 6)
MONTH = lambda n: (START_DATE + timedelta(days=30 * n)).strftime("%b %Y")


class StrategistAgent(BaseAgent):
    role = "Strategist"
    title = "Go-To-Market план: запуск ADME/Tox Predictor в РФ"

    def run(self) -> str:
        audits = self._read_all_audits()

        plan = self._build_plan()
        path = STRATEGY_DIR / "gtm_plan_russia.md"
        path.write_text(plan, encoding="utf-8")
        return str(path)

    def _read_all_audits(self) -> dict[str, str]:
        result = {}
        for f in AUDIT_DIR.glob("audit_*.md"):
            key = f.stem.replace("audit_", "")
            result[key] = f.read_text(encoding="utf-8")
        return result

    def _month(self, n: int) -> str:
        return (START_DATE + timedelta(days=30 * n)).strftime("%b %Y")
    ...
    def _build_plan(self) -> str:
        ...
        cro_list = "\n".join(f"  - {c['name']} ({c['city']}) — {c['focus']}" for c in RUSSIAN_CRO_COMPANIES)
        pharma_list = "\n".join(
            f"  - {c['name']} ({c.get('city', '')}) — {c['focus']}, revenue {c.get('revenue', 'N/A')}"
            for c in RUSSIAN_PHARMA_COMPANIES if c['type'] == 'pharma'
        )
        grant_list = "\n".join(
            f"  - **{g['org']}** — {g['program']}: {g['amount']} ({g['focus']})"
            for g in RUSSIAN_GRANTS
        )
        vc_list = "\n".join(
            f"  - **{v['name']}** — {v['focus']}, {v['stage']}, чек {v['check']}"
            for v in VCS_RUSSIAN
        )

        return f"""# GTM Plan: ADME/Tox Predictor → Российский рынок
...
| Цель {m12} | 5-15 платящих клиентов, $50-150K ARR |
| Цель {m18} | 20+ клиентов, $500K+ ARR, подготовка к Series A |
| Финансирование | Гранты ₽34M (0-6 мес) → Pre-seed $100K (6-12) → Seed $200-500K (12-18) |
| Команда | 1 (основатель) → 3-5 чел к {m12} |
...
### Фаза v2.2 — Product-Market Fit ({m2}–{m6})
...
### Приоритет 1: CRO (первые пилоты, {m0}–{m4})
...
### Месяц 3–4 ({m2}–{m4}) — Product-Market Fit
...
### Месяц 13–18 ({m13}–{m18}) — Market Leadership
...
*Дата: {START_DATE.strftime("%Y-%m-%d %H:%M")}*
"""
...
if __name__ == "__main__":
    ...
    path = agent.run()
    print(f"GTM Plan generated: {path}")
