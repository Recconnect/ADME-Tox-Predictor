...
### Причина
**start.bat имел проблемы с кодировкой** — русские символы ломали парсинг batch-файла в cmd.exe.

### Решение (2026-07-06)
✅ **start.bat переписан в чистом ASCII** — все русские строки заменены на английские  
...
### Как запустить
1. **Двойной клик на `start.bat`** (теперь работает корректно)
2. Дождаться сообщения "You can now view your Streamlit app"
3. Открыть браузер на http://localhost:8501

### Альтернативные способы
- `start_simple.bat` — упрощённый запуск без venv
- `LAUNCH.html` — интерактивная страница с проверкой статуса
